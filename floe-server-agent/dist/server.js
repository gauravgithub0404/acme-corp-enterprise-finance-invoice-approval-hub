...
if (!AGENT_SHARED_SECRET && !ALLOW_INSECURE_AGENT) {
    console.error('[Floe Server Agent] FATAL: AGENT_SHARED_SECRET is not set. Refusing to start, because this ' +
...
const deploymentManager = new deployment_1.DeploymentManager();
const AGENT_VERSION = '1.0.0';
// -------------------------------------------------------------
// AGENT HEALTH & SYSTEM METRICS
// -------------------------------------------------------------
app.get('/api/v1/health', async (req, res) => {
    const docker = await (0, docker_1.checkDockerAvailability)();
    res.json({
        status: 'online',
...
app.get('/api/v1/system', async (req, res) => {
    try {
        const [cpu, mem, fsSize, docker] = await Promise.all([
            ...
            (0, docker_1.checkDockerAvailability)()
        ]);
        const primaryDisk = fsSize[0] || { size: 0, available: 0 };
        const containers = await (0, docker_1.getRunningContainers)();
        res.json({
            hostname: os_1.default.hostname(),
...
app.listen(PORT, BIND_HOST, () => {
    if (!AGENT_SHARED_SECRET) {
        console.warn(`[Floe Server Agent] WARNING: running with ALLOW_INSECURE_AGENT=true and no AGENT_SHARED_SECRET. ` +
